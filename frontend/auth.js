...
function authNavKullanici(kullanici) {
  ...
  chip.style.cssText = `
    ...
    padding:5px 10px 5px 12px; background:rgba(255,255,255,0.04);
    border:1px solid #30363d; border-radius:20px; white-space:nowrap;
  `;
  chip.innerHTML = `
    ...
    <button id="authLogoutBtn" title="Çıkış Yap" style="
      ...
      font-size:15px; padding:1px 2px; border-radius:4px;
      transition:color 0.15s; line-height:1;
    ">⏻</button>
  `;

  const headerRight = document.querySelector(".header-right");
  if (headerRight) headerRight.appendChild(chip);
  else nav.after(chip);
